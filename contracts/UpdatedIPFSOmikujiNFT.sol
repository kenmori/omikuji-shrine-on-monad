...
contract UpdatedIPFSOmikujiNFT is ERC721, Ownable {
    ...
    function drawOmikuji() external payable returns (uint256) {
        ...
        // 確率分布（合計10000）
        if (randomValue < 10) {
            // 0.1% (10/10000) - Super Ultra Great Blessing
            result = 0;
        } else if (randomValue < 110) {
            // 1.0% (100/10000) - Ultra Great Blessing  
            result = 1;
            ...
            // 33.9% (3390/10000) - Minor Blessing
            result = 6;
        }
    ...
    function getProbabilities() external pure returns (uint16[7] memory) {
        return [10, 100, 500, 1000, 2000, 3000, 3390]; // 0.1%, 1%, 5%, 10%, 20%, 30%, 33.9%
    }
